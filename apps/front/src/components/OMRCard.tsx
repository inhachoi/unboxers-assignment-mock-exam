...
export default function OMRCard() {
  const { answers, setObjectiveAnswer, setSubjectiveAnswer } = useExamStore(
    useShallow((s) => ({
      answers: s.answers,
      setObjectiveAnswer: s.setObjectiveAnswer,
      setSubjectiveAnswer: s.setSubjectiveAnswer,
    })),
  )
    ...
    <div className="flex gap-5 items-start w-full max-w-4xl mx-auto min-w-180">
      <div className="bg-white rounded-2xl shadow-md p-6 flex-1">
        ...
        <div className="text-center mb-6 border-b border-gray-200 pb-4">
          ...
          <h2 className="text-2xl font-extrabold tracking-[0.3em] text-gray-900">
            OMR 카드
          </h2>
          <p className="text-xs text-gray-500 mt-2 leading-relaxed">
            객관식 답안은 터치해서 칠하고, 주관식 답안은 터치한 뒤 키패드로
            입력해요.
          </p>
              ...
              {Array.from({ length: OBJECTIVE_COUNT }, (_, i) => i + 1).map(
                (num) => (
                  <div key={num} className="flex items-center gap-2">
                    <span className="w-6 text-right text-sm font-bold text-gray-500 shrink-0">
                      {num}
                    </span>
