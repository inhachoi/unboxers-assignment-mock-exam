...
export default function ResultScreen() {
  ...
  const objectiveResults = examResult.results.filter(
    (r) => r.answerType === 'objective',
  )
  const subjectiveResults = examResult.results.filter(
    (r) => r.answerType === 'subjective',
  )
  const currentResults =
    tab === 'objective' ? objectiveResults : subjectiveResults
    ...
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col items-center py-12 px-6">
      ...
      <div className="bg-white rounded-2xl shadow-md p-8 w-full max-w-lg mb-6">
        <div className="flex justify-between text-sm text-gray-500 mb-6 pb-4 border-b border-gray-100">
          ...
          <span>
            {studentInfo.school} · {studentInfo.grade}학년 ·{' '}
            {studentInfo.studentNumber}번
          </span>
        </div>
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-2">정답률 {scorePercent}%</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-md w-full max-w-lg mb-6 overflow-hidden">
        ...
        <div className="flex border-b border-gray-100">
          {(['objective', 'subjective'] as Tab[]).map((t) => (
            ...
            >
              {t === 'objective'
                ? `객관식 (${objectiveResults.length}문항)`
                : `주관식 (${subjectiveResults.length}문항)`}
            </button>
          ))}
        </div>
        ...
        <div className="divide-y divide-gray-50">
          {currentResults.map((r) => {
            ...
            const myAnswer =
              r.answerType === 'objective'
                ? answers.objective[r.number]
                : answers.subjective[r.number]
            return (
              <div
                key={r.number}
                className={`flex items-center justify-between px-6 py-3 ${style.bg}`}
              >
                ...
                <span
                  className={`text-sm font-extrabold ${style.color} flex items-center gap-1`}
                >
